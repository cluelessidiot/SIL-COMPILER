...
%token DECL ENDDECL INT STR STAR
...
Declarations :	DECL DeclList ENDDECL { PrintSymbolTable(); declrflag=1; }
...
VarList : VarList ',' I_D {}
	  | I_D 	{}
	  ;
I_D	: ID {  if(declrflag==1)
                Install($1->varname,current_type,1,1,0); }
	  |ID'['NUM']' {
	  	if(declrflag==1)	
	 	 Install($1->varname,current_type,$3->val,$3->val,0);}
	  | ID '['NUM']''['NUM']'{
	  	if(declrflag==1)
	  	Install($1->varname,current_type,$3->val*$6->val,$3->val,$6->val);
	  	}
	  | MUL ID {
	  if(declrflag==1)
	  Install($2->varname,current_type,1,1,0);
