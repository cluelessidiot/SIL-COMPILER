...
{  
  ...
  case NODE_VAR:{
			...
			if( (t->left && ((t->left->val) >= (temp->row))) || (t->right && ((t->right->val) >= (temp->column)))){
			...
			break;
				...
				if(t->left && (t->left->nodetype != NODE_CONSTANT)){
					yyerror("Array size must be a constant");
					exit(1);
				}
				if(t->right && (t->right->nodetype != NODE_CONSTANT)){
					yyerror("Array size must be a constant");
					exit(1);
				}
			}
...
reg_index getAddr(tnode *t){
	...
	if(t->nodetype == VAR_POINTER){ //Pointer type variable.
		p=getReg();
...
struct tnode* createTree(int val,int type,char* varname,int nodetype,struct tnode *l,struct tnode *m,struct tnode *r){
    ...
    if(varname!=NULL && nodetype!=NODE_VAR)
    {
    ...
    *(temp->varname)=varname;
    ...
    temp->right = r;
    temp->mid =m;
  //  if(nodetype!=NODE_VAR && nodetype!=VAR_POINTER)
typecheck(temp);
    return temp;
}
